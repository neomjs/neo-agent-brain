...
| **B4 ⭐** | **SAFETY-CRITICAL — runtime writes to `AiConfig`** (see §4) | `[guarded: check-aiconfig-test-mutation]` — **PARTIAL**: the guard runs B4 as two halves against ONE catalog id. The **DB-path subset GATES** (`storagePaths` / `database` / `collections` / `logPath`); the **full B4 scope is REPORT-ONLY** (`598` writes across `146` leaves measured in `test/**`, printed on every run, not gating). Both scan `test/**` only, so `ai/**` remains unenforced (§4). The halves are `scope` fields on the executable rules, never distinct ids: the catalog's id grammar is a single `B4`, so a suffixed id (`B4-DB-PATH`) cannot be expressed as a row | tests isolate by construction (`UNIT_TEST_MODE`); NEVER mutate the shared singleton |
