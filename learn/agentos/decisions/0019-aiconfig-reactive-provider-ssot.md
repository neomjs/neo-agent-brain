...
| Attribute | Value |
|---|---|
| **Status** | Accepted — 2026-06-04 (graduated at Discussion #12453; published by PR #12458, and the reactive Provider it decides is live on `dev` — `ai/config.mjs` registers the Tier-1 singleton over 307 `leaf()` declarations in `ai/configBase.mjs`). **Amended 2026-09-25 (#508) — the B4 row's guard claim was a false assurance and now says so:** the rule carrying the B4 id detected the DB-path subset only, so auditing "is B4 enforced?" against this table returned a wrong answer. The row is now marked **PARTIAL** with both halves named, the broader-clause detector ships as a report-only rule that prints its live measured count on every run, and promoting it to gating waits on the specs holding the bulk of those writes growing a real injection story rather than a per-leaf escape marker. No count is recorded here by design — §4's tag contract keeps point-in-time figures with the owning guard, which is the only thing that can re-measure them. |
...
| **B4 ⭐** | **SAFETY-CRITICAL — runtime writes to `AiConfig`** (see §4) | `[guarded: check-aiconfig-test-mutation]` — **PARTIAL**: the guard runs B4 as two halves against ONE catalog id. The **DB-path subset GATES** (`storagePaths` / `database` / `collections` / `logPath`); the **B4 clause's broader surface is REPORT-ONLY** — assignment writes and `Object.assign` calls through a `*Config` root, printed as a live count on every run, not gating. Both scan `test/**` only, so `ai/**` remains unenforced (§4). The halves are `scope` fields on the executable rules, never distinct ids: the catalog's id grammar is a single `B4`, so a suffixed id (`B4-DB-PATH`) cannot be expressed as a row. **Run the guard for the current count** (§4) — the number is deliberately absent here, since a tag that carries a point-in-time census goes stale the moment the surface moves. A subtree bound to a local first (`const s = AiConfig.x; s.y = 1`) and computed keys are out of static reach by construction. | tests isolate by construction (`UNIT_TEST_MODE`); NEVER mutate the shared singleton |
