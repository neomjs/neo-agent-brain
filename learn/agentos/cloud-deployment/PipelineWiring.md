...
```bash
docker compose -f cloud/deploy/docker-compose.yml --profile cloud config | grep -E 'NEO_REF|NEO_REVISION'
```
...
docker compose -f cloud/deploy/docker-compose.yml exec mc-server cat /app/.neo-revision
```
...
The deployment's persistent state (`cloud/deploy/docker-compose.yml`):
