...
```bash
docker compose -f deploy/cloud/docker-compose.yml --profile cloud config | grep -E 'NEO_REF|NEO_REVISION'
```
...
docker compose -f deploy/cloud/docker-compose.yml exec mc-server cat /app/.neo-revision
```
...
The deployment's persistent state (`deploy/cloud/docker-compose.yml`):
