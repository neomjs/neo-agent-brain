...
For pull, each route maps one non-overlapping territory to one `extractorId`. `RawRepoSource` emits raw whole-file chunks. `ParserSource` resolves a tenant-local parser and emits that parser's structured chunks; its canonical route options contain exactly `parserId` and `parserVersion`. `ConversationCorpusSource` is the first specialized built-in: it reads the org corpus that `github-content-sync` publishes through the route's own reader — the root `_index.json` plus the `<origin>/{issues,pulls,discussions,archive}` trees — and emits per-element conversation chunks whose ownership is the corpus repository's tuple while the conversation's origin rides `customMeta` (neo-agent-brain#402); its only route option is `origins`, an optional list of index `repoSlug` values that narrows the selection. `ReleaseNotesCorpusSource` is the same corpus's second route: it reads each origin's `<origin>/release-notes/_index.json`, whose items name every note's `path`, and emits one `release` chunk per note, with the same `origins` option. Specialized built-in or operator-installed extractors can own richer repository semantics. Descriptor id/version and parser id/version remain distinct provenance—a chunk must never claim parser execution that did not happen.
