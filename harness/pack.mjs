...
export const TREE_EXCLUDES = Object.freeze([
    ...
    'scripts/diagnostics/genesisProbe.mjs'
]);
 ...
 * `config.mjs` AND each per-server `ai/mcp/server/<name>/config.mjs`) can carry hand-edited
 ...
 * `cloud/package.json`, and a scan that cannot see it hard-errors exactly where a broken
 ...
 * @param {Object} [options.brainPackageJson] Parsed repo cloud/package.json (Brain-tier authority).
...
export function stageOrganism({stageDir = STAGE_DIR, electronVersion} = {}) {
    ...
    const
        ...
        brainPath        = path.join(repoRoot, 'cloud/package.json'),
    ...
    // Config overlays are tracked wrappers now (neo-agent-brain#13); nothing to generate at pack time.
