...
Prerequisite for the Brain legs: a fresh per-clone `config.mjs` — if the orchestrator child
...
`resolveBrainPaths` re-resolves the leaves through `config.mjs` itself under the profile env,
...
top-level `config.mjs` AND every per-server MCP overlay, all of which can carry hand-edited
