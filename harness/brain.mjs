...
export const ORCHESTRATOR_ENTRY = 'cloud/daemons/orchestrator/daemon.mjs';
export const FLEET_SERVER_ENTRY = 'cloud/services/fleet/devFleetServer.mjs';
...
export function loadFleetRuntimeContracts(repoRoot = DEFAULT_REPO_ROOT) {
    ...
    if (!contract) {
        contract = Promise.all([
            import(pathToFileURL(path.join(absoluteRoot, 'cloud/graph/normalizeAgentIdentityNodeId.mjs')).href),
            import(pathToFileURL(path.join(absoluteRoot, 'cloud/services/fleet/fleetLaunchContract.mjs')).href),
            import(pathToFileURL(path.join(absoluteRoot, 'cloud/services/fleet/fleetWireMethods.mjs')).href)
 ...
 * one-shot system-Node child imports `config.mjs` under the given env and prints the resolved
