 ...
 * a string-literal predicate misses, and it is a live site (`scripts/lifecycle/inflightLock.mjs`).
 *
...
export const ALLOWLIST = Object.freeze({
    A1: new Set([
        'daemons/wake/daemon.mjs',
        'cloud/services/fleet/devFleetServer.mjs'
    ]),
    A5: new Set(),
    ...
    'PLANE-LITERAL': new Set([
        ...
        "scripts/diagnostics/bootstrapCodexSandbox.mjs::export const DEFAULT_SQLITE_DIR = '.neo-ai-data/sqlite';",
        ...
        "cloud/daemons/orchestrator/taskDefinitions.mjs::chromaDataDir = '.neo-ai-data/chroma/unified',"
    ]),
    B3: new Set([
        'mcp/server/BaseServer.mjs',
        'cloud/mcp/server/shared/logger.mjs'
    ])
});
...
function main() {
    ...
    const files    = rawFiles
        ...
        .filter(f => ['cloud/', 'daemons/', 'mcp/', 'scripts/', 'services/', 'shared/', 'test/'].some(tree => f.startsWith(tree)));
