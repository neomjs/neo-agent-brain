...
/**
 * The two hand-maintained places this lint compares. Fixtures are minimal on purpose: the guard's
...
const run = identities => checkEngineCoherence({identities, modelStats: MODEL_STATS});
...
test.describe('lint-identity-engine-coherence — agreement passes', () => {
    test('both places agreeing is clean, and the resident is counted as checked', () => {
    ...
    test('non-active and non-agent entries are skipped entirely', () => {
        ...
        expect(run([benched])).toEqual({violations: [], checked: 0})
    });
...
test.describe('lint-identity-engine-coherence — the RED cases (each place, one at a time)', () => {
    ...
    test('both disagreements report separately rather than collapsing into one', () => {
        const {violations} = run([
            identity('@neo-one', 'Anthropic Claude Opus 4.8 Agent Identity', {releaseDate: '2026-05-28'})
        ]);

        expect(violations.map(v => v.kind).sort()).toEqual(['description', 'releaseDate'])
    });
});
...
test.describe('lint-identity-engine-coherence — declared absence is NOT drift', () => {
    ...
    // value. A rotation-explicit description is honest, not disagreement.
    test('a rotating seat with a rotation-explicit description passes', () => {
        const {violations, checked} = run([
...
test.describe('lint-identity-engine-coherence — the live repo', () => {
    test('the committed registry and ModelStats agree', () => {
