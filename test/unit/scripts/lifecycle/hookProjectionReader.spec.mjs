...
test.describe('hookProjectionReader — pure bounded hook rendering', () => {
    test.beforeAll(async () => {
        ({buildComputedRouteResult} = await import('../../../../cloud/services/graph/computedRouteResult.mjs'));
        ({buildLifecycleFrontier}   = await import('../../../../cloud/services/graph/lifecycleFrontier.mjs'));
        ({makeHookProjectionWriter} = await import('../../../../cloud/services/memory-core/hookProjectionWriter.mjs'));
        ({readHookProjection}       = await import('../../../../scripts/lifecycle/hookProjectionReader.mjs'))
    });
