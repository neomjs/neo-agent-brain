...
// from the canonical Fleet harness-type authority. Every entry carries the three
...
// order (register the harness type first; the Body's pickers/cards derive from that same entry).
{
    ...
    for (const type of Object.keys(HARNESS_LAUNCH_CONTRACTS)) {
        if (!registered.has(type)) {
            throw new Error(`deriveHarnessLaunchSpec: launch contract '${type}' is not a registered harness type (src/fleet/contract/harnessTypes.mjs). Register the type there first — the launch vocabulary must stay a subset of the authority.`);
        }
    }
}
