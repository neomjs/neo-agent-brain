...
class KbReconciliationService extends Base {
    ...
    async reconcileTenant({tenantId, repoSlug, collection, orphanVersionGap, autoTombstone}) {
        try {
            const currentVersion  = await this.fetchTenantConfigVersion(tenantId),
                  ...
                  declaredByRepo  = await this.fetchTenantExtractionIdentities(tenantId),
                  // Only the nested proof-bound snapshot answers what the completed current profile
                  // yielded. `pathsAfterPush` remains physical deletion authority in manifestDiff.
                  extractionSnapshotsByRepo = Object.fromEntries(
                      Object.entries(manifestsByRepo || {})
                          .filter(([, manifest]) => manifest?.extractionSnapshot)
                          .map(([repo, manifest]) => [repo, manifest.extractionSnapshot])
                  ),
                  configDiff     = diffTenantChunks({rows, currentVersion, orphanVersionGap}),
                  manifestDiff   = diffTenantManifest({rows, manifestsByRepo}),
                  extractionDiff = diffTenantExtractionIdentity({
                      rows,
                      tenantId,
                      declaredByRepo,
                      extractionSnapshotsByRepo
                  }),
                  diff = this.combineDiffs({configDiff, manifestDiff, extractionDiff});
            ...
            logger.info(`[KbReconciliationService] tenant ${tenantId}: ${diff.staleCount} config-stale chunk(s), ${diff.manifestOrphanCount} manifest-orphan chunk(s), ${diff.extractionOrphanCount} extraction-currency orphan chunk(s), ${diff.actionableCount} actionable, ${tombstonedCount} tombstoned (autoTombstone=${autoTombstone})`)
        } catch (err) {
    ...
    /**
     * @summary Test-stubbable seam — current server-derived extraction identity per tenant repo.
     * @param {String} tenantId
     * @returns {Promise<Object<String, {extractionIdentity: String}>>}
     * @protected
     */
    async fetchTenantExtractionIdentities(tenantId) {
        const config         = await IngestionService.getTenantConfig({tenantId}),
              declaredByRepo = {};

        for (const repo of config?.tenantRepos || []) {
            if (
                typeof repo?.repoSlug === 'string'
                && repo.repoSlug
                && typeof repo?.extractionIdentity === 'string'
                && /^[a-f0-9]{64}$/u.test(repo.extractionIdentity)
            ) {
     ...
     * @protected
     */
    combineDiffs({configDiff, manifestDiff, extractionDiff} = {}) {
        const actionableIds = [...new Set([
            ...(configDiff?.actionableIds || []),
            ...(manifestDiff?.actionableIds || []),
            ...(extractionDiff?.actionableIds || [])
        ])];
