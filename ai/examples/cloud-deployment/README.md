...
| [`pre-push-hook.sh`](./pre-push-hook.sh) | A git `pre-push` hook that resolves both Brain CLIs below an explicit absolute `NEO_AGENTOS_RUNTIME_ROOT`, streams changed files through `ai:kb-push-client` when `NEO_KB_MCP_URL` is configured, and otherwise uses the local `ai:ingest-tenant` bulk facade. |
