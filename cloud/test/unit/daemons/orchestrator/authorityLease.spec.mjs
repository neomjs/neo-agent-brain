...
} from '../../../../daemons/orchestrator/authorityLease.mjs';
import {FileLeaseHeldError} from '../../../../daemons/shared/fileLease.mjs';
...
test.describe('#16230 — orchestrator wiring: lease before receipt, refusal is silent-free', () => {
    ...
    test('a lease lost mid-reprobe ABORTS the unfreeze success pipeline: no ledger event, no tombstone, no "unfrozen" report', async () => {
        const {getFreezeRecord, upsertFreezeRecord}  = await import('../../../../services/memory-core/helpers/freezeRecordStore.mjs');
        const {HEAL_LEDGER_DIR_NAME, readHealLedger} = await import('../../../../services/memory-core/helpers/healEventLedgerStore.mjs');
    ...
    test('the deployment snapshot honors shouldWrite at its effect boundary (fenced, never written)', async () => {
        const {DeploymentStateBridgeService} = await import('../../../../daemons/orchestrator/services/DeploymentStateBridgeService.mjs');
