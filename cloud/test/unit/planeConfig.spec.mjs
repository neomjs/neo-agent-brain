...
import ConfigProvider, {createConfigProxy} from '../../ConfigProvider.mjs';
import {
    CANONICAL_PLANE_ID,
    UNKNOWN_PLANE_ID,
...
} from '../../../mcp/server/neural-link/configBase.mjs';
...
test.describe('plane-member derivation witnesses — #15791 seat-variance ground truth', () => {
    ...
    test('knowledge-base + neural-link members derive from the same anchor', async () => {
        ...
        const templateModule = await import('../../config.template.mjs');

        if (!Neo.ai?.Config) {
            Neo.ai        = Neo.ai || {};
            Neo.ai.Config = templateModule.default;
        }
        const KbConfigBase = (await import('../../mcp/server/knowledge-base/configBase.mjs')).default;
...
test.describe('derivePlaneMemberPaths — the completeness half (#15932)', () => {
    ...
    test('the derived set EQUALS each declared PLANE_MEMBER_PATHS — all four declaring configs', async () => {
        ...
        expect(new Set(derivePlaneMemberPaths({descriptorData: McConfigBase.config.data, anchor})))
            .toEqual(new Set(McConfigBase.PLANE_MEMBER_PATHS ?? (await import('../../mcp/server/memory-core/configBase.mjs')).PLANE_MEMBER_PATHS));
        ...
        const KbConfigBase                          = (await import('../../mcp/server/knowledge-base/configBase.mjs')).default,
              {PLANE_MEMBER_PATHS: KB_MEMBER_PATHS} = await import('../../mcp/server/knowledge-base/configBase.mjs');
...
test.describe('⭐ one env ⇒ one resolved default across every declaring config base', () => {
    ...
    test('every leaf binding NEO_MEMORY_DB_PATH resolves the SAME absolute default', async () => {
        ...
        const kb    = await import('../../mcp/server/knowledge-base/configBase.mjs'),
              nl    = await import('../../../mcp/server/neural-link/configBase.mjs'),
